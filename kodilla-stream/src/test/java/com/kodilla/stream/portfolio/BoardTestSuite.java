...
public class BoardTestSuite {
    ...
    void testAddTasksListAverageWorkingOnTask() {
        ...
        int resultSum = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
        ...
        long resultCount = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
                .count();

        double result = (double) resultSum / resultCount;
    ...
    private Board prepareTestData() {
        ...
        User user4 = new User("developer3", "JKonrad Bridge");
